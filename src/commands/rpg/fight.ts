...
export default {
    ...
    async callback({ message, args }): Promise<any> {
        if (message.mentions.users.first()) {
            ...
            const mob = args[0];
            if (!/(dragon)|(ogre)|(gremlin)|(imp)|(spirit)|(goblin)/.test(mob))
                return message.channel.send("Unrecognized mob type.");
...
async function fightMob(user: User, enemy: any, message: Message): Promise<any> {
    ...
    const power = (bestWeapon.base * toPower[bestWeapon.rarity] * (1 - bestArmor.base)) > 0 ? bestWeapon.base * toPower[bestWeapon.rarity] * (1 - bestArmor.base) : 3; // current equation to calculate damage
    ...
    if (reply) switch(reply.content) {
        case "attack":
            ...
            if (player.health <= 0) {
                ...
                message.channel.send(`You died. You lost ${player.coins / 5} coins and a ${toEmoji[randomItem.rarity]} ${randomItem.rarity} ${randomItem.name}`);
